...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if original_health > 0 and self.health <= 0:
    ...
    def action(self, gamestate):
        if self.turns_to_chew > 0:
            ...
            if self.place.bees:
                ...
                if chewed_bee:
                    damage = chewed_bee.health 
                    chewed_bee.reduce_health(damage)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        stand_place = self.place.exit 
        while stand_place is not None:
            if stand_place.ant is not None:
                stand_place.ant.double()
            stand_place = stand_place.exit
        # END Problem 12
